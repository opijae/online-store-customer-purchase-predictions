...
def make_lgb_oof_prediction(train, y, test, features,args='test', categorical_features='auto', model_params=None, folds=10):
    ...
    if type(args)== str:
        mlflow.start_run(run_name=args)
    else:
        mlflow.start_run(run_name=args.version)
    ####################MLFLOW###########################
...
def remove_sepcial(data):
    df=data.copy()
    special_case=[ 'TEST', 'PADS', 'ADJUST', 'ADJUST2','TEST001','TEST002','SP1002']
    temp=[]
    for pid,price in zip(df['product_id'],df['total']):
        # print(pid,price)
        if pid in special_case:
            temp.append(0)
        else:
            temp.append(price)
    df['total'] = temp
    ...
    for customer in tqdm(df.customer_id.unique()):
        transaction=df[df.customer_id==customer].price.to_numpy()
        q1,q3=np.percentile(transaction,[10,90])
        iqr=q3-q1
        lower_bound=q1-(iqr*1.5)
        upper_bound=q3+(iqr*1.5)
        upper_bound=max(upper_bound,300)
        index=df[(df.price>upper_bound)&(df.customer_id==customer)].index
        df.drop(index,inplace=True)
        o_idx.extend(index)
    np.savetxt('outline.csv',np.array(o_idx),delimiter=",")
    return df
if __name__ == '__main__':
    ...
    parser.add_argument('--version', type=str, default='basecode', help="test_version")
    ...
    if model == 'baseline1': # baseline 모델 1
        ...
        y_oof, test_preds, fi = make_lgb_oof_prediction(train, y, test, features,args, model_params=model_params)
    else:
        test_preds = baseline_no_ml(data, year_month)
    
    fi.to_csv('fi.csv',sep=',')
    # 테스트 결과 제출 파일 읽기
    ...
    try:
        ...
        np.savetxt("probability.csv", test_preds, delimiter=",")
    
    
    os.makedirs(output_dir, exist_ok=True)
    # 제출 파일 쓰기
    sub.to_csv(os.path.join(output_dir , f'output_{args.version}.csv'), index=False)